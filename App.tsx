...
import { Search, AlertCircle, History, BarChart3, Palette, Microscope, Cast, Globe, Sun, Moon, Key, CreditCard, ExternalLink, DollarSign, TrendingUp, MonitorPlay, Briefcase } from 'lucide-react';

const App: React.FC = () => {
  const [showIntro, setShowIntro] = useState(true);
  const [topic, setTopic] = useState('');
  // Aspect ratio is now hardcoded to 16:9 in the service calls
  const [complexityLevel, setComplexityLevel] = useState<ComplexityLevel>('General Audience');
  const [visualStyle, setVisualStyle] = useState<VisualStyle>('Broadcast');
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setLoadingMessage(`Researching market data for ${topic}...`);
    ...
    try {
      ...
      setLoadingStep(2);
      setLoadingMessage(`Producing broadcast visuals...`);
      ...
      if (err.message && (err.message.includes("Requested entity was not found") || err.message.includes("404") || err.message.includes("403"))) {
          ...
          setError('The broadcast service is temporarily unavailable. Please try again.');
      }
    } finally {
      setIsLoading(false);
      setLoadingStep(0);
  ...
  const handleEdit = async (editPrompt: string) => {
    ...
    setError(null);
    setLoadingStep(2);
    setLoadingMessage(`Refining visuals: "${editPrompt}"...`);
            ...
            <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed font-medium">
                        InvestMediaPro uses premium Gemini 3 Pro models which are not available on the free tier.
                    </p>
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200 font-sans selection:bg-cyan-500 selection:text-white pb-20 relative overflow-x-hidden animate-in fade-in duration-1000 transition-colors">
      ...
      {/* Background Elements */}
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-slate-50 to-white dark:from-slate-900 dark:via-slate-950 dark:to-black z-0 transition-colors"></div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 md:h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 md:gap-4 group">
            <div className="relative scale-90 md:scale-100">
                <div className="absolute inset-0 bg-blue-500 blur-lg opacity-20 dark:opacity-40 group-hover:opacity-60 transition-opacity"></div>
            ...
            <div className="flex flex-col">
                ...
                InvestMedia<span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-600 dark:from-blue-400 dark:to-cyan-400">Pro</span>
                </span>
                <span className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 font-medium">Broadcast Visual Intelligence</span>
            </div>
          </div>
        ...
        <div className={`max-w-6xl mx-auto transition-all duration-500 ${imageHistory.length > 0 ? 'mb-4 md:mb-8' : 'min-h-[50vh] md:min-h-[70vh] flex flex-col justify-center'}`}>
          ...
          {!imageHistory.length && (
            <div className="text-center mb-6 md:mb-16 space-y-3 md:space-y-8 animate-in slide-in-from-bottom-8 duration-700 fade-in">
              <div className="inline-flex items-center justify-center gap-2 px-4 py-1.5 rounded-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-amber-600 dark:text-amber-300 text-[10px] md:text-xs font-bold tracking-widest uppercase shadow-sm dark:shadow-[0_0_20px_rgba(251,191,36,0.1)] backdrop-blur-sm">
                <MonitorPlay className="w-3 h-3 md:w-4 md:h-4" /> Best in class storytelling
              </div>
              <h1 className="text-3xl sm:text-5xl md:text-8xl font-display font-bold text-slate-900 dark:text-white tracking-tight leading-[0.95] md:leading-[0.9]">
                Broadcast <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-cyan-600 to-indigo-600 dark:from-blue-400 dark:via-cyan-400 dark:to-indigo-400">Your Story.</span>
              </h1>
              <p className="text-sm md:text-2xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto font-light leading-relaxed px-4">
                Generate professional-grade infographics and diagrams instantly grounded with real-time Google Search data.
              </p>
            </div>
          )}
          ...
          <form onSubmit={handleGenerate} className={`relative z-20 transition-all duration-300 ${isLoading ? 'opacity-50 pointer-events-none scale-95 blur-sm' : 'scale-100'}`}>
            ...
            <div className="relative group">
                <div className="absolute -inset-1 bg-gradient-to-r from-blue-500 via-cyan-500 to-indigo-500 rounded-3xl opacity-10 dark:opacity-20 group-hover:opacity-30 dark:group-hover:opacity-40 transition duration-500 blur-xl"></div>
                ...
                <div className="relative bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 p-2 rounded-3xl shadow-2xl">
                    ...
                    <div className="relative flex items-center">
                        <Search className="absolute left-4 md:left-6 w-5 h-5 md:w-6 md:h-6 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
                        <input
                            type="text"
                            value={topic}
                            onChange={(e) => setTopic(e.target.value)}
                            placeholder="What market or topic are you covering?"
                    ...
                    {/* Controls Bar - Optimized for Mobile Grid */}
                    <div className="grid grid-cols-2 md:flex md:flex-row gap-2 p-2 mt-2">
                    
                    {/* Level Selector */}
                    <div className="col-span-1 md:flex-1 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-200 dark:border-white/5 px-4 py-3 flex items-center gap-3 hover:border-cyan-500/30 transition-colors relative overflow-hidden group/item">
                        <div className="hidden sm:block p-2 bg-white dark:bg-slate-800 rounded-lg text-blue-600 dark:text-blue-400 shrink-0 shadow-sm">
                            <Briefcase className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col z-10 w-full overflow-hidden">
                            <label className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-wider">Expertise</label>
                            <select 
                    ...
                    {/* Style Selector */}
                    <div className="col-span-1 md:flex-1 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-200 dark:border-white/5 px-4 py-3 flex items-center gap-3 hover:border-purple-500/30 transition-colors relative overflow-hidden group/item">
                         <div className="hidden sm:block p-2 bg-white dark:bg-slate-800 rounded-lg text-purple-600 dark:text-purple-400 shrink-0 shadow-sm">
                            <Palette className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col z-10 w-full overflow-hidden">
                            <label className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-wider">Format</label>
                            <select 
                                value={visualStyle} 
                            ...
                            >
                                ...
                                <option value="Studio">3D Studio</option>
                     ...
                     {/* Language Selector */}
                     <div className="col-span-1 md:flex-1 bg-slate-50 dark:bg-slate-950/50 rounded-2xl border border-slate-200 dark:border-white/5 px-4 py-3 flex items-center gap-3 hover:border-green-500/30 transition-colors relative overflow-hidden group/item">
                         <div className="hidden sm:block p-2 bg-white dark:bg-slate-800 rounded-lg text-green-600 dark:text-green-400 shrink-0 shadow-sm">
                            <Globe className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col z-10 w-full overflow-hidden">
                            <label className="text-[9px] md:text-[10px] font-bold text-slate-500 uppercase tracking-wider">Language</label>
                            <select 
                                value={language} 
                    ...
                    {/* Generate Button - Mobile Full Width in grid or last cell */}
                    <div className="col-span-1 flex flex-col gap-1 w-full md:w-auto">
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full md:w-auto h-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-8 py-4 rounded-2xl font-bold font-display tracking-wide hover:brightness-110 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] whitespace-nowrap flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
                        >
                    ...
                    {/* Mobile helper text below grid */}
                    <div className="md:hidden text-center mt-2">
                         <span className="text-[9px] text-slate-400 font-medium uppercase tracking-wider opacity-70">16:9 HD Broadcast Generation</span>
                    </div>
                </div>
            </div>
          </form>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-6">
                    {imageHistory.slice(1).map((img) => (
                        <div 
                            ...
                            className="group relative cursor-pointer rounded-2xl overflow-hidden border border-slate-200 dark:border-white/10 hover:border-blue-500/50 transition-all shadow-lg bg-white dark:bg-slate-900/50 backdrop-blur-sm"
                        >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-4 pt-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                                ...
                                <div className="flex gap-2">
                                    {img.level && <span className="text-[9px] text-blue-100 uppercase font-bold tracking-wide px-1.5 py-0.5 rounded-full bg-blue-900/60 border border-blue-500/20">{img.level}</span>}
                                </div>
                            </div>
                        </div>
                    ))}
