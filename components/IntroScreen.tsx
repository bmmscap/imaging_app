...
import { Play, BarChart3, Globe, TrendingUp, Briefcase, Activity } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] bg-slate-950 flex flex-col items-center justify-center overflow-hidden font-display">
      ...
      <div className="relative w-64 h-64 sm:w-80 sm:h-80 md:w-96 md:h-96 flex items-center justify-center perspective-1000">
        ...
        <div className={`relative w-40 h-40 md:w-48 md:h-48 transition-all duration-1000 preserve-3d ${phase >= 2 ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`}>
          {/* Wireframe Sphere */}
          <div className="absolute inset-0 border border-blue-500/30 rounded-full animate-[globe-spin_10s_linear_infinite]"></div>
          ...
          {phase === 1 && strands.map((_, i) => (
            ...
            >
               ...
               >
                 <div className="w-[1px] h-24 bg-gradient-to-t from-blue-500 to-transparent absolute bottom-0 left-1/2 -translate-x-1/2"></div>
               </div>
            </div>
          ))}
        </div>
        ...
        <div className={`absolute flex items-center justify-center transition-all duration-1000 ease-out will-change-transform ${phase >= 2 ? 'scale-100 opacity-100' : 'scale-50 opacity-0'}`}>
           ...
           {/* The Graphic Card */}
           <div className="relative w-32 h-48 md:w-40 md:h-56 bg-black border border-blue-500/30 rounded-xl shadow-2xl overflow-hidden flex flex-col items-center pt-6 md:pt-8 gap-3 group">
              {/* Scan Line */}
              <div className="absolute w-full h-1 bg-blue-400/50 blur-[2px] animate-[scan-line_3s_linear_infinite]"></div>
              
              {/* Holographic Grid Background */}
              <div className="absolute inset-0 opacity-20 bg-[linear-gradient(rgba(59,130,246,0.2)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.2)_1px,transparent_1px)] bg-[size:10px_10px]"></div>

              {/* Icon Content */}
              <div className="w-12 h-12 md:w-16 md:h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center shadow-lg relative z-10">
                  <BarChart3 className="w-6 h-6 md:w-8 md:h-8 text-white" />
              </div>

              {/* Data Lines */}
              <div className="w-20 md:w-24 space-y-2 relative z-10">
                  <div className="h-1 w-full bg-slate-800 rounded overflow-hidden"><div className="h-full bg-blue-500 w-3/4 animate-pulse"></div></div>
                  <div className="h-1 w-full bg-slate-800 rounded overflow-hidden"><div className="h-full bg-cyan-500 w-1/2 animate-pulse delay-75"></div></div>
           ...
           {phase >= 2 && particles.map((_, i) => {
             ...
             return (
                <div 
                    key={i}
                    className="absolute w-1 h-1 bg-blue-400 rounded-full transition-all duration-1000 delay-500"
                    style={{ 
                ...
                >
                    ...
                    <div 
                        className={`absolute h-[1px] bg-blue-800/30 origin-right transition-all`}
                        style={{ 
                            width: `${radius}px`,
                            right: '50%', 
                            top: '50%', 
      ...
      <div className={`absolute bottom-16 md:bottom-20 flex flex-col items-center transition-all duration-1000 px-6 ${phase === 3 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
         <h1 className="text-3xl md:text-5xl font-bold text-white mb-2 tracking-tight text-center">
            InvestMedia<span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400">Pro</span>
         </h1>
         
         <div className="flex flex-col items-center gap-3 mb-8 text-center">
            <p className="text-slate-400 text-xs md:text-sm uppercase tracking-[0.3em]">Storytelling. Elevated.</p>
            
            {/* Google Search Grounding Badge */}
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-950/40 border border-blue-500/30 backdrop-blur-md animate-in fade-in zoom-in duration-1000 delay-500 shadow-[0_0_15px_rgba(59,130,246,0.2)]">
                <Globe className="w-3 h-3 text-blue-400" />
                <span className="text-[9px] md:text-[10px] font-bold text-blue-100 uppercase tracking-wider">Now with Google Search Grounding</span>
            </div>
         </div>
         ...
         >
            <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-blue-600 to-cyan-600 opacity-20 group-hover:opacity-100 transition-opacity duration-300"></div>
            <div className="absolute inset-0 border border-blue-500/50 rounded-full group-hover:border-blue-400 transition-colors"></div>
            
            <div className="relative flex items-center gap-3">
                <span className="text-blue-300 group-hover:text-white font-bold tracking-widest text-sm transition-colors">INITIALIZE SYSTEM</span>
                <Play className="w-4 h-4 text-blue-300 group-hover:text-white fill-current transition-colors" />
            </div>
         </button>
      </div>
