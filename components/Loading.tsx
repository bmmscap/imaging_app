...
import { Loader2, LineChart, Briefcase, BarChart3, TrendingUp, PieChart, Layers, Globe, DollarSign, Activity, Zap } from 'lucide-react';
...
const Loading: React.FC<LoadingProps> = ({ status, step, facts = [] }) => {
  ...
  const FlyingItem = ({ delay, position, type, content }: { delay: number, position: number, type: 'icon' | 'text', content: any }) => {
    ...
    return (
      <div 
        className={`absolute flex items-center justify-center font-bold opacity-0 select-none ${type === 'text' ? 'text-blue-600 dark:text-blue-400 text-[10px] md:text-xs tracking-[0.2em] bg-white/80 dark:bg-slate-900/80 border border-blue-500/30 px-2 py-0.5 md:px-3 md:py-1 rounded shadow-[0_0_10px_rgba(59,130,246,0.3)] backdrop-blur-sm' : 'text-amber-500 dark:text-amber-400'}`}
        style={{
    ...
    <div className="relative flex flex-col items-center justify-center w-full max-w-4xl mx-auto mt-8 min-h-[350px] md:min-h-[500px] overflow-hidden rounded-3xl bg-white/40 dark:bg-slate-900/40 border border-slate-200 dark:border-white/10 shadow-2xl backdrop-blur-md transition-colors">
      ...
      <style>{`
        ...
        @keyframes pulse-core {
          ...
          70% { box-shadow: 0 0 0 30px rgba(59, 130, 246, 0); transform: scale(1.05); }
      ...
      <div className="relative z-20 mb-10 md:mb-16 scale-[0.65] md:scale-125 mt-4 md:mt-10">
        {/* Outer Rings */}
        <div className="absolute inset-0 w-64 h-64 -translate-x-[4.5rem] -translate-y-[4.5rem] border border-dashed border-blue-700/30 dark:border-blue-900/50 rounded-full animate-[spin-slow_20s_linear_infinite]"></div>
        ...
        {/* Glowing Center */}
        <div className="relative bg-white/50 dark:bg-white/10 p-1 rounded-full shadow-[0_0_60px_rgba(59,130,246,0.4)] animate-[pulse-core_2s_infinite]">
           <div className="bg-white dark:bg-slate-950 p-4 rounded-full flex items-center justify-center w-24 h-24 relative overflow-hidden border border-blue-500/50">
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500 to-indigo-600 opacity-10 dark:opacity-30"></div>
              <LineChart className="w-12 h-12 text-blue-600 dark:text-blue-300 animate-pulse relative z-10" />
              {/* Inner beams */}
              <div className="absolute top-0 left-1/2 w-[1px] h-full bg-blue-400/50 animate-[spin-slow_2s_linear_infinite]"></div>
              <div className="absolute top-1/2 left-0 h-[1px] w-full bg-blue-400/50 animate-[spin-slow_2s_linear_infinite]"></div>
           </div>
        </div>
        ...
        <div className="absolute top-1/2 left-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] -translate-x-1/2 -translate-y-1/2 pointer-events-none">
           <FlyingItem content={Briefcase} type="icon" delay={0} position={1} />
           <FlyingItem content="MARKETS" type="text" delay={0.2} position={2} />
           <FlyingItem content={TrendingUp} type="icon" delay={0.5} position={3} />
           <FlyingItem content="TRENDS" type="text" delay={0.7} position={4} />
           <FlyingItem content={PieChart} type="icon" delay={1.0} position={5} />
           ...
           <FlyingItem content={Layers} type="icon" delay={2.0} position={9} />
      ...
      <div className="relative z-30 w-full max-w-lg bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-2xl border border-slate-200 dark:border-white/10 text-center flex flex-col items-center transition-all duration-500 min-h-[140px] md:min-h-[160px]">
        ...
        <div className="flex items-center gap-3 mb-4">
            ...
            {step === 2 && <Zap className="w-4 h-4 text-blue-600 dark:text-blue-400 animate-spin" />}
            {step >= 3 && <Activity className="w-4 h-4 text-emerald-500 dark:text-emerald-400 animate-bounce" />}
            <h3 className="text-blue-700 dark:text-blue-400 font-bold text-[10px] md:text-xs tracking-[0.2em] uppercase font-display">
            {status}
            </h3>
        </div>
        ...
        <div className="w-full h-1 bg-slate-200 dark:bg-slate-800 mt-6 rounded-full overflow-hidden">
            <div 
              className="h-full bg-gradient-to-r from-amber-400 via-blue-500 to-emerald-500 transition-all duration-1000 ease-out relative overflow-hidden shadow-[0_0_10px_rgba(59,130,246,0.8)]"
